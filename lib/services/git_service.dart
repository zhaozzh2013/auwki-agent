...
class GitService {
  ...
  /// D01：本地分支列表。
  static Future<List<String>> branches({String? path}) async {
    final r = await _run(
  ...
  static Future<String> createBranch(String name, {String? path}) async {
    final r = await _run(['branch', name], path: path);
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  }

  static Future<String> switchBranch(String name, {String? path}) async {
    final r = await _run(['switch', name], path: path);
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  }

  static Future<String> deleteBranch(String name, {String? path}) async {
    final r = await _run(['branch', '-d', name], path: path);
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  }

  /// D02：单个文件相对 HEAD 的 diff（含暂存与未暂存）。
  static Future<String> fileDiff(String file, {String? path}) async {
    final r = await _run(['diff', 'HEAD', '--', file], path: path);
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  }

  /// D03：冲突文件列表。
  static Future<List<String>> conflictedFiles({String? path}) async {
    final r = await _run(
  ...
  /// D04：提交历史图（graph + oneline）。
  static Future<String> graph({int count = 30, String? path}) async {
    final r = await _run(
      ['log', '--graph', '--oneline', '--decorate', '-n', '$count'],
      path: path,
    );
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  ...
  /// D05：按作者统计（shortlog）。
  static Future<Map<String, int>> authorStats({String? path}) async {
    final r = await _run(
      ['shortlog', '-sne', '--no-merges', '-n', 'HEAD'],
      path: path,
    );
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
  ...
  /// D05：最近 30 天每日提交数。
  static Future<Map<String, int>> dailyStats({String? path}) async {
    final r = await _run(
      [
        'log',
        '--since=30.days',
  ...
  /// D07：远程同步。
  static Future<String> fetch({String? path}) async {
    final r = await _run(['fetch'], path: path);
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  }

  static Future<String> pull({String? path}) async {
    final r = await _run(['pull'], path: path);
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  }

  static Future<String> push({String? path}) async {
    final r = await _run(['push'], path: path);
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  }

  /// D08：单文件提交历史。
  static Future<String> fileLog(String file, {String? path}) async {
    final r = await _run(
      ['log', '--oneline', '-n', '20', '--', file],
      path: path,
    );
    if (r.exitCode != 0) throw GitServiceException(r.stderr.trim());
    return r.stdout.trim();
  ...
  /// D08：两个版本之间单文件 diff。
  static Future<String> fileDiffBetween(
    String file,
    String revA,
    String revB, {
    String? path,
  ...
  /// D09：读取/写入 .gitignore。
  static Future<String> readGitignore({String? path}) async {
    final root = await repoRoot(path: path);
    if (root == null) return '';
    final f = File('$root/.gitignore');
    return await f.exists() ? await f.readAsString() : '';
  }

  static Future<void> writeGitignore(String content, {String? path}) async {
    final root = await repoRoot(path: path);
    if (root == null) throw GitServiceException('not a git repo');
