...
class AgentRunner {
  ...
  /// 优先使用用户自定义子代理（A16）；找不到时回退内置/临时 agent。
  static CollaborativeAgent agentFromPlanItemWithCustom(
    FlagshipPlanItem item,
    List<Map<String, dynamic>> customAgents,
  ) {
    for (final m in customAgents) {
      if ((m['id'] ?? '').toString() == item.agentId) {
        final title = (m['name'] ?? '').toString().trim();
        final instruction = (m['instruction'] ?? '').toString().trim();
        return CollaborativeAgent(
          id: item.agentId,
          title: title.isEmpty ? item.agentId : title,
    ...
    return agentFromPlanItem(item);
  }

  /// 解析 AI 输出中的工具调用块
  /// 格式: [正式输出]\nwebfetch("...")\n... [输出结束]
  static List<AgentToolCall> parse(String text, {Set<String>? extraTools}) {
    ...
    if (extraTools != null) allowed.addAll(extraTools);
    var cursor = 0;
  ...
  static List<AgentToolCall> _parseCalls(
    ...
    final calls = <AgentToolCall>[];
    // 允许同一行出现多个调用；参数可以是 "..."，也可以是无参 ()。
    // 宽容解析：容忍行首的列表符号（- / *），容忍工具名大小写混用。
    final head = RegExp(r'(?:[-*]\s*)?([A-Za-z_][A-Za-z0-9_]*)\s*\(\s*("|\))');
    var i = 0;
    while (i < block.length) {
      final match = head.firstMatch(block.substring(i));
      if (match == null) break;
      final tool = match.group(1)!.toLowerCase();
      final hasArg = match.group(2) == '"';
      if (hasArg) {
        final argStart = i + match.end;
        final argEnd = _findArgEnd(block, argStart, allowed);
        ...
        if (allowed.contains(tool)) {
          calls.add(AgentToolCall(tool: tool, args: ''));
        }
        i = i + match.end; // 跳过闭合右括号
      }
    }
    return calls;
  }
  ...
  /// 内容里的 `")`（如 CSS url("x")、content: ")"）会被当成普通内容：
  /// 只有“后面是块尾或另一个合法工具调用”的引号才算真正收尾。
  /// 协议转义只认 `\"` 与 `\\`，避免 CSS 反斜杠（如 \2014）吃掉后续字符。
  static int _findArgEnd(
    String s,
    int start,
    Set<String> allowed,
  ) {
    var i = start;
    final toolHead = RegExp(
    ...
    while (i < s.length) {
      ...
      if (c == '"') {
        ...
        if (j >= s.length || s[j] == ')') {
          ...
          final rest = s.substring(k);
          if (rest.trim().isEmpty) return i;
          final m = toolHead.firstMatch(rest);
          if (m != null && allowed.contains(m.group(1)!.toLowerCase())) {
            return i;
          }
          // 内容里的 `")`，继续向后找真正收尾的引号。
        }
...
''';
  ...
  static Future<FlagshipAgentPlan> planFlagshipSession({
    ...
    final system = _flagshipCoordinatorPrompt(
      isEnglish: isEnglish,
  ...
  static String _flagshipCoordinatorPrompt({
    required bool isEnglish,
    List<Map<String, dynamic>>? customAgents,
  }) {
    final extra = _customAgentsPrompt(customAgents, isEnglish: isEnglish);
...
''';
  ...
  static String _customAgentsPrompt(
    List<Map<String, dynamic>>? customAgents, {
    required bool isEnglish,
  }) {
    final agents = customAgents ?? const [];
    final lines = <String>[];
    for (final m in agents) {
      ...
      lines.add('- $label: ${instruction.isEmpty ? 'custom agent' : instruction}');
    }
    if (lines.isEmpty) return '';
    return isEnglish
        ? 'Additional custom agentId values you may select:\n${lines.join('\n')}'
        : '可选择的额外自定义 agentId：\n${lines.join('\n')}';
  }
...
''';
  ...
  static Future<AgentResult> execute(
    ...
    try {
      final out = switch (call.tool) {
        ...
        'writefile' => await _writefile(call.args, cwd, extraAllowedDirs),
        'replacefile' => await _replacefile(call.args, cwd, extraAllowedDirs),
        'command' => await _command(call.args, cwd),
        'screenshot' => await _screenshot(call.args, cwd),
        'sql' => await _sql(call.args, cwd, extraAllowedDirs),
        'zip' => await _zip(call.args, cwd, extraAllowedDirs),
        'unzip' => await _unzip(call.args, cwd, extraAllowedDirs),
        'desktop_dump' => await _desktopDump(call.args),
        'desktop_click' => await _desktopClick(call.args),
        'desktop_type' => await _desktopType(call.args),
        'desktop_key' => await _desktopKey(call.args),
        'desktop_open' => await _desktopOpen(call.args),
        'desktop_scroll' => await _desktopScroll(call.args),
        'desktop_wait' => await _desktopWait(call.args),
        'desktop_ocr' => await _desktopOcr(),
        _ => throw Exception(
  ...
  static Future<String> _webfetch(String url) async {
    ...
    final resp = await NetClient.build()
        .get(uri)
    ...
    // 强制按 UTF-8 解码，避免中文网页被默认 Latin-1 解码成乱码。
    ...
    // A09：HTML 页面自动提取正文，去导航/广告噪音。
    final text = looksHtml ? HtmlTextExtractor.extract(body) : body;
      ...
      try {
        ...
        return results
            ...
            .map((r) {
              ...
              final title = _clip((m['title'] ?? '').toString().trim(), 120);
  ...
  static Future<String> _screenshot(String arg, String? cwd) async {
    ...
    final dir = Directory('$base/screenshots');
    await dir.create(recursive: true);
    final file =
        '${dir.path}/screenshot-${DateTime.now().millisecondsSinceEpoch}.png';
    final script = '''
...
''';
    try {
      ...
      if (r.exitCode != 0) {
        return '${I18n.t('agent.error.screenshot_failed')}\n${r.stderr}';
      }
    } catch (e) {
      return '${I18n.t('agent.error.screenshot_failed')}\n$e';
    }
    return await File(file).exists() ? file : I18n.t('agent.error.screenshot_failed');
  }

  /// 桌面操作工具：让纯文本模型通过“屏幕文本地图 + 坐标操作”控制 Windows。
    /// Desktop control tools: turn the screen into a text map so any model
  /// (including pure-text models) can operate the desktop.
  /// Platform adapters live in desktop_control.dart (Windows/macOS/Linux).
  static bool isDesktopTool(String tool) => switch (tool) {
    'desktop_dump' ||
    'desktop_click' ||
    'desktop_type' ||
    'desktop_key' ||
  ...
  static Future<String> _desktopDump(String arg) =>
      DesktopControl.run('desktop_dump', arg);

  static Future<String> _desktopClick(String arg) =>
      DesktopControl.run('desktop_click', arg);

  static Future<String> _desktopType(String arg) =>
      DesktopControl.run('desktop_type', arg);

  static Future<String> _desktopKey(String arg) =>
      DesktopControl.run('desktop_key', arg);

  static Future<String> _desktopOpen(String arg) =>
      DesktopControl.run('desktop_open', arg);

  static Future<String> _desktopScroll(String arg) =>
      DesktopControl.run('desktop_scroll', arg);

  static Future<String> _desktopWait(String arg) =>
      DesktopControl.run('desktop_wait', arg);

  static Future<String> _desktopOcr() =>
      DesktopControl.run('desktop_ocr', '');

  static Future<String> _sql(
    String arg,
    String? cwd, [
    List<String>? extraAllowedDirs,
  ]) async {
    final parts = _splitToolArg(arg, 2);
    if (parts == null) return I18n.t('agent.error.sql_args');
    final file = File(_resolvePath(parts[0], cwd));
    final safety = _validateReadPath(
      file.path,
      cwd,
      extraAllowedDirs: extraAllowedDirs,
    );
    if (safety != null) return safety;
    final query = parts[1].trim();
    final lower = query.toLowerCase();
    if (!lower.startsWith('select') && !lower.startsWith('with')) {
      return I18n.t('agent.error.sql_readonly');
    }
    if (!await file.exists()) {
      return I18n.t('agent.error.file_missing', {'path': file.path});
    }
    if (file.path.toLowerCase().endsWith('.csv')) {
      return _queryCsv(file, query);
    }
    try {
      ...
      try {
        ...
        for (final row in rows.take(100)) {
          lines.add(headers.map((h) => '${row[h]}').join(' | '));
        }
  ...
  static String _queryCsv(File file, String query) {
    try {
      ...
      for (final line in lines.skip(1).take(100)) {
        out.add(line);
      }
      if (lines.length > 101) out.add('... ${lines.length - 1} rows total');
      return out.join('\n');
    } catch (e) {
      return '[CSV error] $e';
    }
  ...
  static Future<String> _zip(
    ...
    final parts = _splitToolArg(arg, 2);
    if (parts == null) return I18n.t('agent.error.zip_args');
    final src = _resolvePath(parts[0], cwd);
    final dst = _resolvePath(parts[1], cwd);
    final safety = _validateWritePath(
      dst,
      cwd,
      extraAllowedDirs: extraAllowedDirs,
    );
    if (safety != null) return safety;
    if (!await FileSystemEntity.type(src, followLinks: false).then(
      (t) => t != FileSystemEntityType.notFound,
    )) {
      return I18n.t('agent.error.file_missing', {'path': src});
    }
    try {
      final ProcessResult r;
      final norm = src.replaceAll('\\', '/');
      final slash = norm.lastIndexOf('/');
      final parent = slash < 0 ? '.' : norm.substring(0, slash);
      final name = slash < 0 ? norm : norm.substring(slash + 1);
      if (Platform.isWindows) {
      ...
      return I18n.t('agent.done.zip', {'path': dst});
    } catch (e) {
      return '${I18n.t('agent.error.zip_failed')}\n$e';
    }
  }

  static Future<String> _unzip(
    String arg,
    String? cwd, [
    List<String>? extraAllowedDirs,
  ]) async {
    final parts = _splitToolArg(arg, 2);
    if (parts == null) return I18n.t('agent.error.unzip_args');
    final archive = _resolvePath(parts[0], cwd);
    final dest = _resolvePath(parts[1], cwd);
    final safety = _validateWritePath(
      dest,
      cwd,
      extraAllowedDirs: extraAllowedDirs,
    ...
    if (!await File(archive).exists()) {
      return I18n.t('agent.error.file_missing', {'path': archive});
    }
    try {
      ...
      final entries = (list.stdout as String).split('\n');
      for (final line in entries) {
        final entry = line.trim();
        if (entry.isNotEmpty && isUnsafeZipEntry(entry)) {
      ...
      return I18n.t('agent.done.unzip', {'path': dest});
    } catch (e) {
      return '${I18n.t('agent.error.unzip_failed')}\n$e';
    }
  }

  /// 判断 zip 条目是否危险（zip-slip / 绝对路径 / 盘符）。
  static bool isUnsafeZipEntry(String entry) {
    final e = entry.replaceAll('\\', '/');
    if (e.startsWith('/')) return true;
    if (RegExp(r'^[A-Za-z]:/').hasMatch(e)) return true;
    return e.split('/').contains('..');
  }

  static String? _validateReadPath(
    String path,
    String? cwd, {
    List<String>? extraAllowedDirs,
  }) {
    if (_isUnsafePath(path, cwd, extraAllowedDirs: extraAllowedDirs)) {
  ...
  static String _ellipsize(String s, int max) {
    ...
    final omitted = s.length - head.length - tail.length;
    return '$head\n…[中间省略 $omitted 字符]…\n$tail';
  }
  ...
  static List<String> _shellArgs(String cmd) {
    if (Platform.isWindows) {
      // 先把控制台输出编码切到 UTF-8，避免中文输出被按 GBK 解码成乱码。
      return [
        '-NoProfile',
        '-NonInteractive',
        '-Command',
  ...
  static Future<String> _listfiles(String path, String? cwd) async {
    ...
    await for (final entity in dir.list(followLinks: false)) {
      ...
      var p = entity.path.replaceAll('\\', '/');
      // 优先输出相对路径，减少占用上下文 token。
      if (p.startsWith('$base/')) p = p.substring(base.length + 1);
      entries.add('$p$suffix');
  ...
  static Future<String> _writefile(
    ...
    // 支持 path|||content 与 path|||content|||overwrite 两种格式。
    final parts3 = _splitToolArg(arg, 3);
    final isOverwriteForm = parts3 != null &&
        parts3[2].trim().toLowerCase() == 'overwrite';
    final parts = isOverwriteForm ? parts3 : _splitToolArg(arg, 2);
    if (parts == null) return I18n.t('agent.error.write_args');
    final file = File(_resolvePath(parts[0], cwd));
    final safetyIssue = _validateWritePath(
      file.path,
      cwd,
      extraAllowedDirs: extraAllowedDirs,
    );
    if (safetyIssue != null) return safetyIssue;
    final content = _decodePart(parts[1]);
    // F09：写入内容拦截（勒索/键盘记录/窃取凭据等明显恶意模式）。
    final contentIssue = _validateWriteContent(content);
    if (contentIssue != null) return contentIssue;
    final overwrite = isOverwriteForm;
  ...
  static Future<String> _replacefile(
    String arg,
    String? cwd, [
    List<String>? extraAllowedDirs,
  ]) async {
    ...
    final oldText = _decodePart(parts[1]);
    final newText = _decodePart(parts[2]);
    final text = await file.readAsString();
    final span = _findReplaceSpan(text, oldText);
    if (span == null) {
      return I18n.t('agent.error.replace_missing', {'path': file.path});
    }
    // 容错匹配时保留首行缩进，避免替换后丢格式。
    final indent = span.$3
        ? (RegExp(r'^[ \t]*').firstMatch(text.substring(span.$1))?.group(0) ??
              '')
        : '';
    final next = text.replaceRange(span.$1, span.$2, '$indent$newText');
    await file.writeAsString(next);
    return I18n.t('agent.done.replace', {'path': file.path});
  }

  /// 在 text 中定位 oldText 的替换区间 (start, end)。
  /// 先精确匹配；失败后做容错匹配：
  /// 1) 忽略 \r 差异（\r\n 与 \n 混用）；
  /// 2) 忽略行首尾空白差异（逐行 trim 后对齐）。
  /// 第三个字段标记是否为容错匹配（容错匹配需保留首行缩进）。
  /// 返回的是原文中的区间，找不到返回 null。
  static (int, int, bool)? _findReplaceSpan(String text, String oldText) {
    if (oldText.isEmpty) return null;
    final exact = text.indexOf(oldText);
    if (exact >= 0) return (exact, exact + oldText.length, false);

    // 容错 1：忽略 \r（\r\n 与 \n 混用）
    if (text.contains('\r') || oldText.contains('\r')) {
      final t = text.replaceAll('\r', '');
      final o = oldText.replaceAll('\r', '');
      final i = t.indexOf(o);
      if (i >= 0) {
        final start = _mapIndexWithoutCr(text, i);
        final end = _mapIndexWithoutCr(text, i + o.length);
        return (start, end, true);
      }
    }

    // 容错 2：逐行 trim 比较（忽略行首尾空白，容忍换行/缩进细节差异）
    final tLines = text.split('\n');
    final oLines = oldText
        .replaceAll('\r', '')
        .split('\n')
        .map((l) => l.trim())
        .toList();
    final first = oLines.first;
    for (var i = 0; i + oLines.length <= tLines.length; i++) {
      if (tLines[i].trim() != first) continue;
      var ok = true;
      for (var j = 1; j < oLines.length; j++) {
        if (tLines[i + j].trim() != oLines[j]) {
          ok = false;
          break;
        }
      }
      if (ok) {
        final start = _lineStartOffset(tLines, i);
        final end = _lineEndOffset(tLines, i + oLines.length - 1);
        return (start, end, true);
      }
    }
    return null;
  ...
  /// 把“去掉所有 \r 的文本”中的下标映射回原文下标。
  static int _mapIndexWithoutCr(String original, int index) {
    var cr = 0;
    for (var i = 0; i < original.length; i++) {
      if (i - cr >= index) break;
      if (original.codeUnitAt(i) == 13) cr++;
    }
    return index + cr;
  }

  static int _lineStartOffset(List<String> lines, int index) {
    var offset = 0;
  ...
  static int _lineEndOffset(List<String> lines, int index) =>
      _lineStartOffset(lines, index) + lines[index].length;

  static List<String>? _splitToolArg(String arg, int count) {
    if (count <= 1) return null;
    final firstSep = arg.indexOf('|||');
    if (firstSep < 0) return null;
    final first = arg.substring(0, firstSep);
    final rest = arg.substring(firstSep + 3);
    if (count == 2) return [first, rest];
    // count == 3：path ||| body ||| flag（body 允许包含 |||，取最后一个分隔符）。
    final lastSep = rest.lastIndexOf('|||');
    if (lastSep < 0) return null;
    return [
      first,
      rest.substring(0, lastSep),
      rest.substring(lastSep + 3),
    ];
  }
  ...
  /// 解码单个工具参数段：支持 `base64:`/`b64:` 前缀（超长/特殊字符内容），
  /// 否则按协议转义解码。
  static String _decodePart(String s) {
    final t = s.trimLeft();
    if (t.startsWith('base64:') || t.startsWith('b64:')) {
      final b64 = t.substring(t.indexOf(':') + 1).replaceAll(RegExp(r'\s'), '');
      try {
  ...
  static String? _validateCommand(String cmd) {
    ...
    final blocked = <RegExp>[
      ...
      // 不可逆的 git 破坏性操作：交给用户从 Git 面板手动执行。
      RegExp(r'(^|\s)git\s+reset\s+--hard\b'),
      RegExp(r'(^|\s)git\s+clean\s+-\w*f\w*d\w*'),
      // Windows 递归删除 / 整盘格式化。
      RegExp(r'(^|\s)(rd|rmdir)\s+[^\n;]*/\s*s\b'),
      RegExp(r'(^|\s)remove-item\b[^\n;]*(recurse|force)'),
      RegExp(r'(^|\s)format\s+[A-Za-z]:'),
    ];
  ...
  /// 公开的命令安全校验（A02 自定义工具等外部调用复用）。
  static String? validateCommand(String cmd) => _validateCommand(cmd);

  /// 写入内容安全检测（F09）：识别勒索软件、键盘记录、
  /// 凭据窃取、隐藏自启动等明显恶意模式。采用保守规则，避免误伤正常代码。
  static final List<RegExp> _blockedContentPatterns = [
    // 键盘记录：低级键盘钩子。
    RegExp(
      r'SetWindowsHookEx\w*\s*\([^\n]{0,200}WH_KEYBOARD',
      caseSensitive: false,
    ),
    // 勒索：索要加密货币 + 解密/解锁提示。
    RegExp(
      r'(bitcoin|btc|wallet)[^\n]{0,80}(decrypt|ransom|unlock)',
      caseSensitive: false,
    ),
    // 勒索：批量重命名文件为加密后缀。
    RegExp(
      r'(foreach|for\s+|loop)[^\n]{0,160}\.(encrypted|crypted|locked)',
      caseSensitive: false,
    ),
    RegExp(
      r'rename[^\n]{0,60}\.(encrypted|crypted|locked)',
      caseSensitive: false,
    ),
    // 凭据窃取：复制浏览器登录数据/密码库。
    RegExp(
      r'(login\s*data|cookies|passwords?)[^\n]{0,80}(chrome|edge|brave|firefox)[^\n]{0,80}(copy|cp\s|robocopy)',
      caseSensitive: false,
    ),
    // 凭据外传：读取后上传到远程。
    RegExp(
      r'(ReadAllBytes|read\s*\(\))[^\n]{0,80}(UploadString|downloadstring|invoke-webrequest|http[s]?://[^\s]{4,})',
      caseSensitive: false,
    ),
    // 隐蔽自启动：注册表 Run 键写入。
    RegExp(
      r'(reg\s+add|SetValue)[^\n]{0,80}(CurrentVersion\\Run|RunOnce)',
      caseSensitive: false,
    ),
    // 远程脚本下载后直接执行。
    RegExp(
      r'(DownloadString|downloadfile|invoke-webrequest)[^\n]{0,60}(iex|invoke-expression|start-process|bash\s+-c)',
      caseSensitive: false,
    ),
  ];

  /// 返回拦截原因；null 表示放行。
  static String? _validateWriteContent(String content) {
    final lower = content.toLowerCase();
    for (final pattern in _blockedContentPatterns) {
      if (pattern.hasMatch(lower)) {
        return I18n.t('agent.error.content_blocked');
      }
    }
    return null;
  }

  static String? _validateWritePath(
    String path,
    String? cwd, {
    List<String>? extraAllowedDirs,
  }) {
    ...
    if (_isUnsafePath(normalized, cwd, extraAllowedDirs: extraAllowedDirs)) {
      return I18n.t('agent.error.path_blocked', {'path': path});
    }
  ...
  static bool _isUnsafePath(
    ...
    final allowedPrefixes = [
      ...
      for (final d in (extraAllowedDirs ?? const <String>[]))
        if (d.trim().isNotEmpty) d.trim().replaceAll('\\', '/'),
