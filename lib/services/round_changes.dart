...
class WorkspaceSnapshot {
  ...
  /// 单次快照读取正文的总字节预算：超出后只记录大小和修改时间，
  /// 避免大工作区在每次发送前全量读取所有文件拖慢请求。
  static const int _maxReadBudget = 32 * 1024 * 1024;
  ...
  static Future<WorkspaceSnapshot> capture({String? rootPath}) async {
    ...
    await _walk(rootDir, root, files, _ReadBudget(_maxReadBudget));
      ...
      await for (final entity in dir.list(followLinks: false)) {
        ...
        try {
          ...
          if (entity is Directory) {
            ...
            await _walk(entity, root, out, budget);
          } else if (entity is File) {
            final rel = normalizePath(entity.path, base: root);
            if (rel.isEmpty) continue;
            final snap = await _snapFile(entity, rel, budget);
            if (snap != null) {
  ...
  static Future<_SnapFile?> _snapFile(
    File file,
    String rel,
    _ReadBudget budget,
  ) async {
    try {
      ...
      // 预算充足时才读取正文：小文件保留内容用于精确行数 diff；
      // 大文件或预算耗尽时只记录大小和修改时间。
      if (stat.size <= _maxContentBytes && budget.remaining > 0) {
      ...
      return _SnapFile(displayPath: rel, size: stat.size, mtimeMs: mtimeMs);
    } catch (_) {
      return null;
    }
  }
  ...
  /// 暴露可回滚的文本文件（相对路径 -> 内容），供 A07 使用。
  Map<String, String> textFiles() {
    final out = <String, String>{};
    _files.forEach((key, snap) {
      if (snap.content != null) {
        out[snap.displayPath] = snap.content!;
      }
    });
    return out;
  }

  /// 文件是否发生变化：内容齐全时优先比较内容，
  /// 否则（大文件或快照预算耗尽）比较大小和修改时间。
  static bool _changed(_SnapFile a, _SnapFile b) {
